...
public class StatsForWeather {
   ...
   public static void main(String[] args) throws IOException {
      ...
      String options="";
      do {
         options=(String) JOptionPane.showInputDialog(null, "Please select a database option:", "Batter Database", JOptionPane.PLAIN_MESSAGE,
         ...
         switch (options) {
            ...
            case "List All Batters":
               for (BatterwStats batDisp:listOfPlayers) {
                  JOptionPane.showMessageDialog(null, "Nationals Batters:\n"+batDisp, "Player #" + batDisp.getJerseyNumber(), JOptionPane.INFORMATION_MESSAGE, batDisp.getImage());
               }
               break;
   ...
   public static void open() throws IOException {
    ...
    public static void addBatter() {
      ...
      do {
         ...
         do {
            input= (String) JOptionPane.showInputDialog(null, "What is the batters last name?", "Last Name", JOptionPane.PLAIN_MESSAGE);
         ...
         do {
            input=JOptionPane.showInputDialog(null, "What is the batters first name?", "first Name", JOptionPane.PLAIN_MESSAGE);
            if (input==null || input.length()==0)
         ...
         do {
            input=(String) JOptionPane.showInputDialog(null, "What side does the batter bat on?", "Bat Dominance", JOptionPane.QUESTION_MESSAGE,null, battingSide, battingSide[0]);
            if (input==null || input.length()==0)
         ...
         np.setBatDominance(input);    // no option
          ...
          do {
             input=JOptionPane.showInputDialog(null, "How many hits did the batter get when the temperature was optimal last year?");
             if (input==null || input.length()==0)
          ...
          do {
             input=JOptionPane.showInputDialog(null, "How many hits does the batter have when the temperature was optimal year?");
             if (input==null || input.length()==0)
         ...
         flag=JOptionPane.showConfirmDialog(null, "Do you want to add another batter?", "ADD ANOTHER?", JOptionPane.YES_NO_OPTION,
   ...
   public static void updateStats() {
      find();
      String[] updateDatabase={"Select attribute to update:", "Jersey Number", "Last Name", "First Name", "Bat Dominance", "Last Years Total Hits", "Last Years Optimal Hits", "Last Years At Bats", "This Years Total Hits", "This Years Optimal Hits", "This Years Total At Bats", "Database Updated"};
      String select;
      if (currentPlayer>=0) {
         BatterwStats curBat=listOfPlayers.get(currentPlayer);
         do {
            select=(String) JOptionPane.showInputDialog(null, "Please select a Database option:", "Database Option", JOptionPane.PLAIN_MESSAGE,
                null, updateDatabase, updateDatabase[0]);
            switch (select) {
               case "Jersey Number":
                 curBat.setJerseyNumber(Integer.parseInt(JOptionPane.showInputDialog(null, "What is the batters new jersey number?", 
               ...
               case "Bat Dominance":
                 curBat.setBatDominance((String)JOptionPane.showInputDialog(null, "What side does the batter bat on?", 
                                         "Bat Dominance (Right:Left:Swtich): "+curBat.getBatDominance(), JOptionPane.QUESTION_MESSAGE, null, battingSide, battingSide[0]));
                 break;
               case "Last Years Total Hits":
                 curBat.setLyTotalHits(Integer.parseInt(JOptionPane.showInputDialog(null, "How many hits did the batter get last year?", 
                                         "Last Years Total Hits: "+curBat.getLyTotalHits(), JOptionPane.QUESTION_MESSAGE)));
                 break;
               case "Last Years Optimal Hits":
                 curBat.setLyOptHits(Integer.parseInt(JOptionPane.showInputDialog(null, "How many hits did the batter get when the temperature was optimal last year?", 
                                         "Last Years Optimal Hits: "+curBat.getLyOptHits(), JOptionPane.QUESTION_MESSAGE)));
                 break;
               case "Last Years At Bats":
                 curBat.setLyAtBats(Integer.parseInt(JOptionPane.showInputDialog(null, "How many at bats did the batter get last year?", 
                                         "Last Years Total Hits: "+curBat.getLyAtBats(), JOptionPane.QUESTION_MESSAGE)));
                 break;
               case "This Years Total Hits":
                 curBat.setTyTotalHits(Integer.parseInt(JOptionPane.showInputDialog(null, "How many hits has this batter gotten this year?", 
                                         "This Years Total Hits: "+curBat.getTyTotalHits(), JOptionPane.QUESTION_MESSAGE)));
                 break; 
               case "This Years Optimal Hits":
                 curBat.setTyOptHits(Integer.parseInt(JOptionPane.showInputDialog(null, "How many hits did the batter get when the temperature was optimal this year?", 
                                         "This Years Optimal Hits: "+curBat.getTyOptHits(), JOptionPane.QUESTION_MESSAGE)));
                 break;
               case "This Years At Bats":
                 curBat.setTyAtBats(Integer.parseInt(JOptionPane.showInputDialog(null, "How many at bats has this batter gotten this year??", 
    ...
    do {
        ...
        for (Row row : sheet) {
            if (row.getCell(0) != null && row.getCell(1) != null &&
                ...
                if (tempColumnIndex > 0 && row.getCell(tempColumnIndex) != null && row.getCell(tempColumnIndex).getCellType() == CellType.NUMERIC) {
                    totalNumberOfGamesInOp = (int) row.getCell(tempColumnIndex).getNumericCellValue();
                } else {
                    System.out.println("Cell for totalNumberOfGamesInOp is not numeric, is empty, or tempColumnIndex is not set");
                }
                break;
            }
        }
      ...
      // Set ng player to 
         ng.setPlayer(listOfPlayers.get(currentPlayer));
      ...
      //Key:Value// //Key is has to be unique, but Value can be same-2 teams in Chicago//
      ...
      //Cast Object To String//
         String question = (String)JOptionPane.showInputDialog(null, "Which stadium are the Washington Nationals playing in?",
      ...
      //To get percipitation I will need the paid API_Key
             ...
             JOptionPane.showMessageDialog(null,"Nationals vs " + question + "\n" + "Current Temperature: " + mainMap.get("temp") + 
             ...
             probOfOptTemp = (double) totalNumberOfGamesInOp / totalNumberOfGames; // Cast to double for floating-point division
             } else {
               probOfOptTemp = 0; //Handle the case where totalNumberOfGames is 0 to avoid division by zero
             }
             if (ng.getGameTemp()<=55.0) 
                probOfOptTemp= totalNumberOfGames/totalNumberOfGamesInOp;
             else if (ng.getGameTemp()>55.0 && ng.getGameTemp()<=65.0) 
                probOfOptTemp= totalNumberOfGames/totalNumberOfGamesInOp;
             else if (ng.getGameTemp()>65.0 && ng.getGameTemp()<=75.0) 
                probOfOptTemp= totalNumberOfGames/totalNumberOfGamesInOp;            
             else if (ng.getGameTemp()>75.0 && ng.getGameTemp()<=85.0) 
                probOfOptTemp= totalNumberOfGames/totalNumberOfGamesInOp;
             else if (ng.getGameTemp()>85.0 && ng.getGameTemp()<=95.0) 
                probOfOptTemp= totalNumberOfGames/totalNumberOfGamesInOp;
             else if (ng.getGameTemp()>95.0 && ng.getGameTemp()<= 105.0) 
                probOfOptTemp= totalNumberOfGames/totalNumberOfGamesInOp;             
             else
                probOfOptTemp=1.0;
             //I need to fix this calulation   
            ...
            /* I can remove this once I refactor prediction logic */                    
             ...
             System.out.println("Total number of games in optimal temperature (totalNumberOfGamesInOp): " + totalNumberOfGamesInOp);
             System.out.println("Game Temperature (ng.getGameTemp()): " + ng.getGameTemp());
             System.out.println("Probability of Optimal Temperature (probOfOptTemp): " + probOfOptTemp);
             //I think the issue has to do with the NationalsPlayer class
             System.out.println(((((ng.getPlayer().getLyOptHits()+ng.getPlayer().getTyOptHits())/
                                         (ng.getPlayer().getLyTotalHits()+ng.getPlayer().getTyTotalHits()))*
                                         ng.getPlayer().getTyBattingAverage())/probOfOptTemp));

             
             listOfGames.add(ng);
             flagNGames=JOptionPane.showConfirmDialog(null, "Do you want to predict another game?", "Predict Another?", JOptionPane.YES_NO_OPTION,
               JOptionPane.PLAIN_MESSAGE)
                    ==JOptionPane.YES_OPTION;
          } catch (IOException e) {
   ...
   public static void save(List<BatterwStats> listOfPlayers) throws IOException {
        ...
        for (Row rowData : rows) {
            ...
            for (int cn = 0; cn < rowData.getLastCellNum(); cn++) {
                ...
                switch (c.getCellType()) {
                    case STRING: cell.setCellValue(c.getStringCellValue()); break;
                    case NUMERIC: cell.setCellValue(c.getNumericCellValue()); break;
    ...
    public static void find() {
        ...
        for (int i = 1; i < playerList.length; i++) {
            ...
            playerList[i] = (i) + ":" + player.getJerseyNumber() + " " + player.getFirstName() + " (" + player.getLastName() + ")";
        }

        String npSelected = (String) JOptionPane.showInputDialog(null, "Please select a Batter:", "Nationals Batter", JOptionPane.PLAIN_MESSAGE, null, playerList, playerList[0]);
        ...
        if (npSelected != null && npSelected.indexOf(":") >= 0) {
            ...
            JOptionPane.showMessageDialog(null, "Nationals Batter:\n" + cb, "Player #" + cb.getJerseyNumber(), JOptionPane.INFORMATION_MESSAGE, cb.getImage());
        } else {
            JOptionPane.showMessageDialog(null, "Batter not selected.");
            currentPlayer = -1;
        }
